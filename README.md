# 👋 About Me

I'm Mohamad Amer, a fresh Electrical Engineering graduate from Bar-Ilan University, specialized in Hardware and VLSI Circuits.

I have experience with hardware design and circuit simulation using Cadence Virtuoso, and I also work with programming and scripting languages such as Verilog, TCL, C, C++, Python, R, and MATLAB.

I'm currently expanding my skills toward full-stack development and looking to grow as an engineer/developer in the high-tech field.

---

## 🛠 Tech Stack

### Hardware & VLSI
![Cadence](https://img.shields.io/badge/Cadence-Virtuoso-blue)
![Verilog](https://img.shields.io/badge/Verilog-HDL-orange)
![TCL](https://img.shields.io/badge/TCL-Scripting-lightgrey)

### Programming
![C](https://img.shields.io/badge/C-Programming-blue)
![C++](https://img.shields.io/badge/C++-Programming-blue)
![Python](https://img.shields.io/badge/Python-Programming-yellow)
![R](https://img.shields.io/badge/R-Programming-blue)
![MATLAB](https://img.shields.io/badge/MATLAB-Programming-orange)
![JavaScript](https://img.shields.io/badge/JavaScript-Programming-yellow)
![TypeScript](https://img.shields.io/badge/TypeScript-Programming-blue)

### Web Development
![HTML5](https://img.shields.io/badge/HTML5-Markup-orange)
![CSS3](https://img.shields.io/badge/CSS3-Styling-blue)
![React](https://img.shields.io/badge/React-Frontend-lightblue)
![Redux](https://img.shields.io/badge/Redux-State%20Management-purple)
![Node.js](https://img.shields.io/badge/Node.js-Backend-green)

### Databases
![MongoDB](https://img.shields.io/badge/MongoDB-Database-green)
![MySQL](https://img.shields.io/badge/MySQL-Database-blue)
![SQL](https://img.shields.io/badge/SQL-Database-lightgrey)

---

## 📌 Projects

### Amazon Product Page Clone
A front-end project where I recreated an Amazon-style product page for a water bottle using only HTML and CSS.

**Technologies used:** HTML, CSS, GitHub, Netlify

**What I practiced:**
- Page structure
- Product page layout
- Responsive design
- Hover and focus effects
- Git/GitHub workflow
